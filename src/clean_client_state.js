...
export default function cleanClientState(state, playerId) {
    if(state.getIn(['champion', 'id']) == playerId.toString())
        return state.remove('crowd').remove('champion').remove('pendingPlayers').set('team','champion');
    ...
    return state.remove('champion').remove('crowd').remove('pendingPlayers').set('team','crowd');
};